...
const StoryBlockConfigurationModal = ({
    ...
    (configuration?: StoryBlockConfiguration) => {
      if (configuration) {
        ...
        setValue(
          ...
          configuration.handleAppearanceManually
        );
  ...
  const onSave = (data: StoryBlockConfigurationFormData) => {
    if (onSubmit) {
      ...
      if (typeof data.handleAppearanceManually === 'boolean') {
        configuration.handleAppearanceManually = data.handleAppearanceManually;
      }
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => {
              return (
                <CheckBox
                  center
                  title={`Handle Appearance${'\n'}Manually`}
                  checked={value}
                  onPress={() => onChange(!value)}
                />
              );
            }}
            name="handleAppearanceManually"
          />
        </View>
      </View>
      <View style={styles.formItemRow}>
